...
def vacancies_hh(vacancies_hh_list):
    vacancies_hh = []
    for vacancy_hh in vacancies_hh_list:
        vacancy_hh_data = {}
...
def all_hh_links():
    vacancies_hh_links = [main_link_1 + '/search/vacancy?area=&st=searchVacancy&text=' + vacancy + '&page='+ str(i) for i in range(0,40)]
    for link in vacancies_hh_links:
        ...
        vacancies_hh(vacancies_hh_list)

# функция для парсинга вакансий с одной страницы superjob.ru
def vacancies_sj(vacancies_superjob_list):
    vacancies_sj = []
    for vacancy_sj in vacancies_superjob_list:
        vacancy_sj_data = {}
...
def all_sj_links():
    vacancies_sj_links = [main_link_2 + '/vacancy/search/?keywords='+vacancy + '&page='+ str(i) for i in range(1,4)]
    for link in vacancies_sj_links:
        ...
        vacancies_sj(vacancies_sj_list)
